...
The initial project idea was to train a Network to generate Joint Angles (3D) based on a 2D Skeleton that is 
generated by the OpenPose library using a Convolutional Neural Network (CNN). One major goal of that project was
to process represent the Motion in an RGB Image to use as an Input.
After having worked out the generation of the data, we came to the conclusion that the problem was too complex to solve
in the given timeframe, especially in finding an network architecture to support the idea. For that reason it was
...
with generating the Motion Images. Diese Motion Images sollen dann durch mehrere Architekturen getestet werden.
...
I also thought punching should be identifiable.    
...
![OpenPoseLogo](.data/images/openposelogo.png)

For preprocessing the Data [OpenPose](https://github.com/CMU-Perceptual-Computing-Lab/openpose) was used to extract
...
for a CNN. The normalisation is based on a reference joint in the hierarchy, which is then used to transform the other 
...
|-------------------------------------|:-----------------------------------:|:------:|:------:|
|![](doc/images/exampleMotion.gif)      |![](./images/customDataExanoke.jpg) |  60    |   25   |
...
For the problem of recognizing and classifying motions based on extracted motion images, which are low in resolution
the Alex
In contrast to the before mentioned models, the other models performed far better than what can be expected.
It is quite clear that in this project the models have overfitted to the data, since train, test and validation data
are taken from the same source and just shuffled. To really understand how well the models work it is necessary to 
compare them based on a better dataset.

# Discussion

The results of this project cannot be said to be successfull in the sense
that none of the trained models generalize well enough to be applied
to a seperate video. While the models might certainly not be ideal to use
on the given problem, similar reasearch projects have used some of these
models as a baseline and had more success.
Working on this project quite a few important factors in training a 
neural network have been shown. I want to discuss personal understanding of
where the problem in this project lie.
...
motions. If this dataset was to be used for anything other than to use on this specific case,
...
![Abbildung 1: Beispieldaten aus HMDB51](doc/images/dataset_comparison.png)

### Normalisation of the data

This project has shown that the normalisation of data is an important factor
to generalize the networks. While two different normalisations have been implemented,
...
This should represent the change over time.
Other parameters to consider would be angles between joints.
